        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
                <a href="{{url('Category/add')}}" class="btn btn-rounded btn-success">Add Category</a>
            </div>
            <div class="card">
                        ...
                        <div class="col-sm-1 p-1">
                            <select id="per_page" name="per_page" class="form-select custom-select">
                                @foreach($perPageOptions as $option)
                                    <option value="{{ $option }}" {{ $option == $categories->perPage() ? 'selected' : '' }}>{{ $option }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <tbody>
                            ...
                            @foreach($categories as $category)
                                <tr>
                                    ...
                                    <td>
                                        <a href="update/{{$category->id}}" class="btn btn-info btn-sm">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <a href="delete/{{$category->id}}" class="btn btn-danger btn-sm">
